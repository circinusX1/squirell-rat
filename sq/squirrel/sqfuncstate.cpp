...
void SQFuncState::Dump(SQFunctionProto *func)
{
    size_t n=0,i;
    int si;
    scprintf(_SC("SQInstruction sizeof %d\n"),(uint32_t)sizeof(SQInstruction));
    scprintf(_SC("SQObject sizeof %d\n"), (uint32_t)sizeof(SQObject));
    scprintf(_SC("--------------------------------------------------------------------\n"));
    ...
    for(si=0;si<func->_nlocalvarinfos;si++){
        ...
        scprintf(_SC("[%d] %s \t%d %d\n"), (uint32_t)lvi._pos,_stringval(lvi._name), (uint32_t)lvi._start_op, (uint32_t)lvi._end_op);
        n++;
    }
    scprintf(_SC("-----LINE INFO\n"));
    for(i=0;i<_lineinfos.size();i++){
        SQLineInfo li=_lineinfos[i];
        scprintf(_SC("op [%d] line [%d] \n"), (uint32_t)li._op, (uint32_t)li._line);
        n++;
    }
    scprintf(_SC("-----dump\n"));
    n=0;
    for(i=0;i<_instructions.size();i++){
        ...
        if(inst.op==_OP_LOAD || inst.op==_OP_DLOAD || inst.op==_OP_PREPCALLK || inst.op==_OP_GETK ){
            ...
            int lidx = inst._arg1;
            scprintf(_SC("[%03d] %15s %d "), (uint32_t)n,g_InstrDesc[inst.op].name,inst._arg0);
            if(lidx >= 0xFFFFFFFF)
                scprintf(_SC("null"));
            else {
                int refidx;
            ...
            scprintf(_SC("[%03d] %15s %d %f %d %d\n"), (uint32_t)n,g_InstrDesc[inst.op].name,inst._arg0,*((SQFloat*)&inst._arg1),inst._arg2,inst._arg3);
        }
    /*  else if(inst.op==_OP_ARITH){
            scprintf(_SC("[%03d] %15s %d %d %d %c\n"),n,g_InstrDesc[inst.op].name,inst._arg0,inst._arg1,inst._arg2,inst._arg3);
        }*/
        else {
            scprintf(_SC("[%03d] %15s %d %d %d %d\n"), (uint32_t)n,g_InstrDesc[inst.op].name,inst._arg0,inst._arg1,inst._arg2,inst._arg3);
        }
        n++;
    }
    scprintf(_SC("-----\n"));
    scprintf(_SC("stack size[%d]\n"), (uint32_t)func->_stacksize);
    scprintf(_SC("--------------------------------------------------------------------\n\n"));
}
#endif

int SQFuncState::GetNumericConstant(const int cons)
{
...
void SQFuncState::SetInstructionParams(int pos,int arg0,int arg1,int arg2,int arg3)
{
    _instructions[pos]._arg0=(unsigned char)*((size_t *)&arg0);
    _instructions[pos]._arg1=(uint32_t)*((size_t *)&arg1);
    _instructions[pos]._arg2=(unsigned char)*((size_t *)&arg2);
    _instructions[pos]._arg3=(unsigned char)*((size_t *)&arg3);
}

void SQFuncState::SetInstructionParam(int pos,int arg,int val)
{
    switch(arg){
        case 0:_instructions[pos]._arg0=(unsigned char)*((size_t *)&val);break;
        case 1:case 4:_instructions[pos]._arg1=(uint32_t)*((size_t *)&val);break;
...
int SQFuncState::CountOuters(int stacksize)
{
    int outers = 0;
...
bool SQFuncState::IsLocal(size_t stkpos)
{
...
void SQFuncState::MarkLocalAsOuter(int pos)
{
