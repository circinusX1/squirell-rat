...
#define sarg1 (*((const uint32_t *)&_i_._arg1))
        ...
        for(;;)
        {
...
#else
                TARGET = (int)((uint32_t)arg1); continue;
#endif
            ...
            case _OP_CALL: {
                ...
                case OT_NATIVECLOSURE: {
                    bool suspend;
                    bool tailcall;
                    _GUARD(CallNative(_nativeclosure(clo), arg3, _stackbase+arg2, clo, (uint32_t)sarg0, suspend, tailcall));
                    if(suspend){
                        _suspended = SQTrue;
            ...
            case _OP_RETURN:
                ...
                continue;
            case _OP_LOADNULLS:{ for(uint32_t n=0; n < arg1; n++) STK(arg0+n).Null(); }continue;
            case _OP_LOADROOT:  {
...
#else
                    val._unVal.nInteger = (int)((uint32_t)arg1);
#endif
                    break;
                case AAT_FLOAT:
                    val._type = OT_FLOAT;
...
bool SQVM::CallNative(SQNativeClosure *nclosure, int nargs, int newbase, SQObjectPtr &retval, uint32_t target,bool &suspend, bool &tailcall)
{
...
bool SQVM::EnterFrame(int newbase, int newtop, bool tailcall)
{
    if( !tailcall ) {
        ...
        ci->_prevstkbase = (uint32_t)(newbase - _stackbase);
        ci->_prevtop = (uint32_t)(_top - _stackbase);
        ci->_etraps = 0;
        ci->_ncalls = 1;
        ci->_generator = NULL;
        ci->_root = SQFalse;
