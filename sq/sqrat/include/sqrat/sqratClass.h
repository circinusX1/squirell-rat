...
public:
    ...
    Class(HSKVM v, const string& className, bool createClass = true) : Object(v, false) {
        if (createClass && !ClassType<C>::hasClassData(v)) {
            __psq->pushregistrytable(v);
            __psq->pushstring(v, "__classes", -1);
            if (SQ_FAILED(__psq->rawget(v, -2))) {
                __psq->newtable(v);
                __psq->pushstring(v, "__classes", -1);
                __psq->push(v, -2);
                __psq->rawset(v, -4);
            }
            __psq->pushstring(v, className.c_str(), -1);
            ClassData<C>** ud = reinterpret_cast<ClassData<C>**>(__psq->newuserdata(v, sizeof(ClassData<C>*)));
            *ud = new ClassData<C>;
            __psq->setreleasehook(v, -1, &cleanup_hook);
            __psq->rawset(v, -3);
            __psq->pop(v, 2);
            ...
            __psq->resetobject(&classObj);

            __psq->newclass(v, false);
            __psq->getstackobj(v, -1, &classObj);
            __psq->addref(v, &classObj); // must addref before the pop!
            __psq->pop(v, 1);
            InitClass(cd);
        }
    }
...
public:
    ...
    Class& Funcb(const SQChar* name, B method) {
        BindFunc(name, &method, sizeof(method), SqMemberFunc(method));
        return *this;
    }
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Class& SquirrelFunc(const SQChar* name, SQFUNCTION func) {
        __psq->pushobject(vm, ClassType<C>::getClassData(vm)->classObj);
        __psq->pushstring(vm, name, -1);
        __psq->newclosure(vm, func, 0);
        __psq->newslot(vm, -3, false);
    ...
    Function GetFunction(const SQChar* name) {
        ClassData<C>* cd = ClassType<C>::getClassData(vm);
        HSQOBJECT funcObj;
        __psq->pushobject(vm, cd->classObj);
        __psq->pushstring(vm, name, -1);
...
/// @cond DEV
    ...
    static int ClassTypeof(HSKVM vm) {
        __psq->pushstring(vm, ClassType<C>::ClassName().c_str(), -1);
        return 1;
    }
    ...
    void InitClass(ClassData<C>* cd) {
        ...
        __psq->settypetag(vm, -1, cd->staticData.Get());

        // add the default constructor
        __psq->pushstring(vm, _SC("constructor"), -1);
        __psq->newclosure(vm, &A::New, 0);
        __psq->newslot(vm, -3, false);

        // add the set table (static)
        HSQOBJECT& setTable = cd->setTable;
        __psq->resetobject(&setTable);
        __psq->pushstring(vm, _SC("__setTable"), -1);
        __psq->newtable(vm);
        __psq->getstackobj(vm, -1, &setTable);
        __psq->addref(vm, &setTable);
        __psq->newslot(vm, -3, true);

        // add the get table (static)
        HSQOBJECT& getTable = cd->getTable;
        __psq->resetobject(&getTable);
        __psq->pushstring(vm, _SC("__getTable"), -1);
        __psq->newtable(vm);
        __psq->getstackobj(vm, -1, &getTable);
        __psq->addref(vm, &getTable);
        __psq->newslot(vm, -3, true);

        // override _set
        __psq->pushstring(vm, _SC("_set"), -1);
        __psq->pushobject(vm, setTable); // Push the set table as a free variable
        __psq->newclosure(vm, &sqVarSet, 1);
        __psq->newslot(vm, -3, false);

        // override _get
        __psq->pushstring(vm, _SC("_get"), -1);
        __psq->pushobject(vm, getTable); // Push the get table as a free variable
        __psq->newclosure(vm, &sqVarGet, 1);
        __psq->newslot(vm, -3, false);

        // add weakref (apparently not provided by default)
        __psq->pushstring(vm, _SC("weakref"), -1);
        __psq->newclosure(vm, &Class::ClassWeakref, 0);
        __psq->newslot(vm, -3, false);

        // add _typeof
        __psq->pushstring(vm, _SC("_typeof"), -1);
        __psq->newclosure(vm, &Class::ClassTypeof, 0);
        __psq->newslot(vm, -3, false);

        // add _cloned
        __psq->pushstring(vm, _SC("_cloned"), -1);
        __psq->newclosure(vm, &Class::ClassCloned, 0);
        __psq->newslot(vm, -3, false);
    ...
    inline void BindAccessor(const SQChar* name, void* var, size_t varSize, SQFUNCTION func, HSQOBJECT table) {
        ...
        __psq->pushobject(vm, table);
        __psq->pushstring(vm, name, -1);

        // Push the variable offset as a free variable
        PVOID varPtr = __psq->newuserdata(vm, static_cast<size_t>(varSize));
    ...
    Class& BindConstructor(SQFUNCTION method, int nParams, const SQChar *name = 0) {
        ...
        __psq->newclosure(vm, method, 0);
        __psq->setparamscheck(vm,nParams + 1,NULL);
        __psq->newslot(vm, -3, false);
        __psq->pop(vm, 1);
        return *this;
    }
    ...
    DerivedClass(HSKVM v, const string& className) : Class<C, A>(v, string(), false) {
        if (!ClassType<C>::hasClassData(v)) {
            ...
            *ud = new ClassData<C>;
            ...
            __psq->pushobject(v, bd->classObj);
            __psq->newclass(v, true);
            __psq->getstackobj(v, -1, &classObj);
            __psq->addref(v, &classObj); // must addref before the pop!
            __psq->pop(v, 1);
...
/// @cond DEV
    ...
    void InitDerivedClass(HSKVM vm, ClassData<C>* cd, ClassData<B>* bd) {
        ...
        HSQOBJECT& setTable = cd->setTable;
        __psq->resetobject(&setTable);
        __psq->pushobject(vm, bd->setTable);
        __psq->pushstring(vm, _SC("__setTable"), -1);
        __psq->clone(vm, -2);
        __psq->remove(vm, -3);
        __psq->getstackobj(vm, -1, &setTable);
        ...
        HSQOBJECT& getTable = cd->getTable;
        __psq->resetobject(&getTable);
        __psq->pushobject(vm, bd->getTable);
        __psq->pushstring(vm, _SC("__getTable"), -1);
        __psq->clone(vm, -2);
        __psq->remove(vm, -3);
        __psq->getstackobj(vm, -1, &getTable);
        ...
        // add weakref (apparently not provided by default)
        __psq->pushstring(vm, _SC("weakref"), -1);
        __psq->newclosure(vm, &Class<C, A>::ClassWeakref, 0);
        __psq->newslot(vm, -3, false);

        // add _typeof
        __psq->pushstring(vm, _SC("_typeof"), -1);
        __psq->newclosure(vm, &Class<C, A>::ClassTypeof, 0);
        __psq->newslot(vm, -3, false);

        // add _cloned
        __psq->pushstring(vm, _SC("_cloned"), -1);
        __psq->newclosure(vm, &Class<C, A>::ClassCloned, 0);
        __psq->newslot(vm, -3, false);
