...
struct popAsInt
{
    ...
    popAsInt(HSKVM vm, int idx)
    {
        ...
        case OT_BOOL:
            bool sqValueb;
            __psq->getbool(vm, idx, &sqValueb);
            value = static_cast<T>(sqValueb);
            break;
        case OT_INTEGER:
            int sqValue;
            __psq->getinteger(vm, idx, &sqValue);
            value = static_cast<T>(sqValue);
            break;
        case OT_FLOAT:
            SQFloat sqValuef;
            __psq->getfloat(vm, idx, &sqValuef);
...
private:
    ...
    struct pushAsInt<T2, true> {
        void push(HSKVM vm, const T2& value) {
            __psq->pushinteger(vm, static_cast<int>(value));
        }
    };
};
...
struct Var<SharedPtr<T> > {
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    Var(HSKVM vm, int idx) {
        if (__psq->gettype(vm, idx) != OT_NULL) {
...
#define SCRAT_FLOAT( type ) \
 ...
 struct Var<type> { \
     ...
     static void push(HSKVM vm, const type& value) { \
         __psq->pushfloat(vm, static_cast<SQFloat>(value)); \
     } \
 }; \
 \
 template<> \
...
struct Var<bool> {
    ...
    static void push(HSKVM vm, const bool& value) {
        __psq->pushbool(vm, static_cast<bool>(value));
    }
};
...
public:
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    Var(HSKVM vm, int idx) {
        __psq->tostring(vm, idx);
        __psq->getstackobj(vm, -1, &obj);
        __psq->getstring(vm, -1, (const SQChar**)&value);
        __psq->addref(vm, &obj);
        __psq->pop(vm,1);
        v = vm;
    }
...
struct Var<string> {
    ...
    Var(HSKVM vm, int idx) {
        const SQChar* ret;
        __psq->tostring(vm, idx);
        __psq->getstring(vm, -1, &ret);
        value = string(ret, __psq->getsize(vm, -1));
        __psq->pop(vm,1);
    }
    ...
    static void push(HSKVM vm, const string& value) {
        __psq->pushstring(vm, value.c_str(), value.size());
    }
};
...
struct Var<std::string> {
    ...
    Var(HSKVM vm, int idx) {
        ...
        __psq->tostring(vm, idx);
        __psq->getstring(vm, -1, &ret);
        value = wstring_to_string(string(ret, __psq->getsize(vm, -1)));
        __psq->pop(vm,1);
    }
    ...
    static void push(HSKVM vm, const std::string& value) {
        ...
        __psq->pushstring(vm, s.c_str(), s.size());
    }
};
...
public:
    ...
    static void push(HSKVM vm, const char* value, int len = -1) {
        __psq->pushstring(vm, string_to_wstring(std::string(value)).c_str(), len);
    }
};
