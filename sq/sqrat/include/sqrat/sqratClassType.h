...
public:
    ...
    static inline ClassData<C>* getClassData(HSKVM vm) {
        __psq->pushregistrytable(vm);
        __psq->pushstring(vm, "__classes", -1);
#ifndef NDEBUG
        SQRESULT r = __psq->rawget(vm, -2);
        assert(SQ_SUCCEEDED(r)); // fails if getClassData is called when the data does not exist for the given VM yet (bind the class)
#else
        __psq->rawget(vm, -2);
#endif
        __psq->pushstring(vm, ClassName().c_str(), -1);
#ifndef NDEBUG
        r = __psq->rawget(vm, -2);
        assert(SQ_SUCCEEDED(r)); // fails if getClassData is called when the data does not exist for the given VM yet (bind the class)
#else
...
#endif
        ClassData<C>** ud;
        __psq->getuserdata(vm, -1, (PVOID*)&ud, NULL);
        __psq->pop(vm, 3);
        return *ud;
    }
    ...
    static inline bool hasClassData(HSKVM vm) {
        if (!getStaticClassData().Expired()) {
            __psq->pushregistrytable(vm);
            __psq->pushstring(vm, "__classes", -1);
            if (SQ_SUCCEEDED(__psq->rawget(vm, -2))) {
                __psq->pushstring(vm, ClassName().c_str(), -1);
                if (SQ_SUCCEEDED(__psq->rawget(vm, -2))) {
                    __psq->pop(vm, 3);
                    return true;
    ...
    static void PushInstance(HSKVM vm, C* ptr) {
        ...
        if (it != cd->instances->end()) {
            __psq->pushobject(vm, it->second);
            return;
        }

        __psq->pushobject(vm, cd->classObj);
        __psq->createinstance(vm, -1);
        __psq->remove(vm, -2);
        __psq->setinstanceup(vm, -1, new std::pair<C*, SharedPtr<typename unordered_map<C*, HSQOBJECT>::type> >(ptr, cd->instances));
        __psq->setreleasehook(vm, -1, &DeleteInstance);
        __psq->getstackobj(vm, -1, &((*cd->instances)[ptr]));
    }

    static void PushInstanceCopy(HSKVM vm, const C& value) {
        __psq->pushobject(vm, getClassData(vm)->classObj);
        __psq->createinstance(vm, -1);
        __psq->remove(vm, -2);
#ifndef NDEBUG
    ...
    static C* GetInstance(HSKVM vm, int idx, bool nullAllowed = false) {
        ...
        {
            if (nullAllowed && __psq->gettype(vm, idx) == OT_NULL) {
                return NULL;
            }
            ...
            if (SQ_FAILED(__psq->getinstanceup(vm, idx, (PVOID*)&instance, classType))) {
...
#else
            __psq->getinstanceup(vm, idx, (PVOID*)&instance, 0);
#endif
        }
        ...
        __psq->gettypetag(vm, idx, (PVOID*)&actualType);
        if (actualType == NULL) {
            int top = __psq->gettop(vm);
            __psq->getclass(vm, idx);
            while (actualType == NULL) {
                __psq->getbase(vm, -1);
                __psq->gettypetag(vm, -1, (PVOID*)&actualType);
            }
            __psq->settop(vm, top);
        }
