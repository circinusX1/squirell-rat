...
public:
    ...
    static void SetInstance(HSKVM vm, int idx, C* ptr)
    {
        ...
        __psq->setinstanceup(vm, idx, new std::pair<C*, SharedPtr<typename unordered_map<C*, HSQOBJECT>::type> >(ptr, cd->instances));
        __psq->setreleasehook(vm, idx, &Delete);
        __psq->getstackobj(vm, idx, &((*cd->instances)[ptr]));
    }
...
public:
    ...
    {
        ClassData<C>* cd = ClassType<C>::getClassData(vm);
        __psq->setinstanceup(vm, idx, new std::pair<C*, SharedPtr<typename unordered_map<C*, HSQOBJECT>::type> >(ptr, cd->instance));
        __psq->setreleasehook(vm, idx, &Delete);
        __psq->getstackobj(vm, idx, &((*cd->instances)[ptr]));
    }
    ...
    static int New(HSKVM vm) {
        ...
        return __psq->throwerror(vm, (ClassType<C>::ClassName() + string(_SC(" constructing is not allowed"))).c_str());
#else
        SQUNUSED(vm);
        return 0;
#endif
    ...
    static int Copy(HSKVM vm, int idx, const void* value) {
        ...
        SQUNUSED(idx);
        SQUNUSED(value);
        return __psq->throwerror(vm, (ClassType<C>::ClassName() + string(_SC(" cloning is not allowed"))).c_str());
    }
