...
private:
    ...
    static void printFunc(HSKVM /*v*/, const SQChar *s, ...)
    {
        va_list vl;
        va_start(vl, s);
        scvprintf(s, vl);
    ...
    static int runtimeErrorHandler(HSKVM v)
    {
        ...
        if(__psq->gettop(v) >= 1)
        {
            ...
            if(SQ_SUCCEEDED(__psq->getstring(v, 2, &sErr)))
            {
                errStr = sErr;
            }
            else
...
public:
    ...
    ///
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    SqratVM(int initialStackSize = 1024, unsigned char libsToLoad = LIB_ALL): m_vm(__psq->open(initialStackSize))
    ...
    {
        __psq->setprintfunc(m_vm, printFunc, errFunc);
    }
    ...
    {
        __psq->newclosure(m_vm, runErr, 0);
        __psq->seterrorhandler(m_vm);
        __psq->setcompilererrorhandler(m_vm, comErr);
    }
