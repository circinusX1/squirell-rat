...
public:
    ...
    static int Func(HSKVM vm) {
        // Get the arg count
        int argCount = __psq->gettop(vm) - 2;

        const SQChar* funcName;
        __psq->getstring(vm, -1, &funcName); // get the function name (free variable)
        ...
        if (SQ_FAILED(__psq->get(vm, 1))) { // Lookup the proper overload
...
#if !defined (SCRAT_NO_ERROR_CHECKING)
        SQRESULT result = __psq->call(vm, argCount + 1, true, ErrorHandling::IsEnabled());
        if (SQ_FAILED(result)) {
            return __psq->throwerror(vm, LastErrorString(vm).c_str());
        }
#else
        __psq->call(vm, argCount + 1, true, ErrorHandling::IsEnabled());
#endif
...
#if !defined (SCRAT_NO_ERROR_CHECKING)
        SQRESULT result = __psq->call(vm, argCount + 1, false, ErrorHandling::IsEnabled());
        if (SQ_FAILED(result)) {
            return __psq->throwerror(vm, LastErrorString(vm).c_str());
        }
#else
        __psq->call(vm, argCount + 1, false, ErrorHandling::IsEnabled());
#endif
