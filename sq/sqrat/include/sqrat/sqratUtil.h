...
public:
    ...
    static void Clear(HSKVM vm) {
        ...
        __psq->pushstring(vm, "__error", -1);
        __psq->rawdeleteslot(vm, -2, false);
        __psq->pop(vm, 1);
    }
    ...
    static void Throw(HSKVM vm, const string& err) {
        ...
        if (SQ_FAILED(__psq->rawget(vm, -2))) {
            __psq->pushstring(vm, "__error", -1);
            string** ud = reinterpret_cast<string**>(__psq->newuserdata(vm, sizeof(string*)));
            *ud = new string(err);
            __psq->setreleasehook(vm, -1, &error_cleanup_hook);
            __psq->rawset(vm, -3);
            __psq->pop(vm, 1);
            return;
        }
...
inline string FormatTypeError(HSKVM vm, int idx, const string& expectedType) {
    ...
    if (SQ_SUCCEEDED(sq_typeof(vm, idx))) {
        ...
        __psq->tostring(vm, -1);
        __psq->getstring(vm, -1, &actualType);
        __psq->pop(vm, 2);
...
inline string LastErrorString(HSKVM vm) {
    const SQChar* sqErr;
    __psq->getlasterror(vm);
    if (__psq->gettype(vm, -1) == OT_NULL) {
        __psq->pop(vm, 1);
        return string();
    }
    __psq->tostring(vm, -1);
    __psq->getstring(vm, -1, &sqErr);
    __psq->pop(vm, 2);
    return string(sqErr);
}
