...
#ifdef _DEBUG
        {
            ...
            for(i = 0;i < nsize; i++) {
                if(exp->_nodes[i].type>MAX_CHAR)
                    scprintf(_SC("[%02d] %10s "), (uint32_t)i,g_nnames[exp->_nodes[i].type-MAX_CHAR]);
                else
                    scprintf(_SC("[%02d] %10c "), (uint32_t)i,exp->_nodes[i].type);
                scprintf(_SC("left %02d right %02d next %02d\n"), (uint32_t)exp->_nodes[i].left, (uint32_t)exp->_nodes[i].right, (uint32_t)exp->_nodes[i].next);
            }
            scprintf(_SC("\n"));
        }
#endif
